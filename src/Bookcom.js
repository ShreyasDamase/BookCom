...
const Bookcom = () => {
  ...
  async function addUser() {
    ...
    console.log(i_token);
    let response = await axios.post('http://ec2-3-7-254-228.ap-south-1.compute.amazonaws.com:8080/Bookcom/user/register', { 'userid': user.id }, { headers: { Authorization: `Bearer ${i_token}` } });
  }
  addUser();
    ...
    <>
      {loading ? (
        <div role="status" className="flex items-center justify-center h-screen bg-black">
          <svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600
